...
export function checkForBackupAndS3Nodes(diagram, setAlertMessage) {
  if (diagram.model.nodeDataArray.length > 0) {
    const hasS3Node = diagram.model.nodeDataArray.some(
      (node) => node.text === "Simple Storage Service (S3)"
    );
    const hasBackUpNode = diagram.model.nodeDataArray.some(
      (node) => node.text === "Backup"
    );
    const backUpGuide =
      "정보, 소프트웨어 및 시스템의 백업 사본은 합의된 백업 주제의 정책에 따라 유지되고 정기적으로 테스트되어야 함. S3, Backup 사용 권장.";
...
export function checkForMonitoringNodes(diagram, setAlertMessage) {
  if (diagram.model.nodeDataArray.length > 0) {
    const hasCloudWatch = diagram.model.nodeDataArray.some(
      (node) => node.text === "CloudWatch"
    );
    const hasCloudTrail = diagram.model.nodeDataArray.some(
      (node) => node.text === "CloudTrail"
    );
    const monitoringGuide =
      "네트워크, 시스템 및 애플리케이션은 비정상적인 동작을 모니터링하고, 잠재적인 정보 보안 사건을 평가하기 위한 적절한 조치를 취해야 함. CloudWatch, CloudTrail 사용 권장.";
...
export async function checkForLogAnalysisNodes(diagram, setAlertMessage) {
  const analysisGuide =
    "활동, 예외, 결함 및 기타 관련 이벤트를 기록하는 로그는 생성되고 저장되며 보호되고 분석되어야 함. 로그 분석 서비스와 로깅 서비스(S3)와 연결하세요.";
  const hasAthena = diagram.model.nodeDataArray.some(
    (node) => node.text === "Athena"
  );
  const hasOpenSearchService = diagram.model.nodeDataArray.some(
...
export async function checkForDbAccess(diagram, setAlertMessage) {
  if (diagram.model.nodeDataArray.length > 0) {
    const hasDb = diagram.model.nodeDataArray.some(
